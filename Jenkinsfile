    ...
    stages{
        ...
        stage('Docker Build') {
            ...
            steps {
                ...
                // 构建镜像
                sh "docker build --build-arg JAR_FILE=`ls target/*.jar | cut -d '/' -f2` -t registry.cn-shenzhen.aliyuncs.com/zk-docker-repos/docker-repos:eventdriver-${BRANCH_NAME}-${eventdriver} ."
                // 推送至仓库
